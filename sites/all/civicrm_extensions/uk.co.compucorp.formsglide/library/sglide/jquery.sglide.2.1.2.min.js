		...
		init: function(options){
			this.each(function(i, el){
				...
				var THE_VALUE		= valueObj[guid] = settings.startAt,
					...
					markers			= (settings.snap.points > 0 && settings.snap.points <= 19 && settings.snap.marks),
				...
				var btn_snap = (settings.snap.points > 0 && settings.snap.points <= 20 && (snapType == 'hard' || snapType == 'soft'));
				...
				var doSnap = function(kind, m){
					if (snaps > 0 && snaps < 20){	// min 1, max 9
						var knobWidth = knob.width(),
							pctFive = self_width * (20-snaps) / 100 - 2;
				...
				$(document).on(mEvt.move+'.'+guid, function(e){
					...
					if (state == 'active'){
						...
						// snap to
						if (snaps > 0 && snaps < 20 && (snapType == 'soft' || snapType == 'hard'))	// min 1, max 9
				...
				var setStartAt = function(e){
					...
					// inits
					if (snaps > 0 && snaps < 20)	drawSnapmarks();
